...
class class_field_list extends class_field {
	...
	// TODOEXPLAIN
	function class_field_list($connection_settings, $fieldsettings) {
	...
	function form_field($row, $m_form, $required_typecheck_result = 0 ) {
		...
		if ( $this->m_dbhandle == null ) {
			// TODOTODO
			$res2 = mysql_query($this->oMisc->PlaceURLParametersInQuery($this->m_query, "no"), $this->oDb->conn) or die(mysql_error());
		} else {
			// TODOTODO
