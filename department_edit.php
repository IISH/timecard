...
if ( !$oWebuser->hasDepartmentAuthorisation() ) {
...
$oPage->setTitle('Timecard | Department Day (edit)');
...
function createAdminDayEditContent( $date  ) {
	...
	// get design
	$design = new class_contentdesign("page_department_day_edit");
...
function getAdminDayEdit( $date, $oShortcutTemplate ) {
	...
	$oForm->add_field( new class_field_list ( $settings, array(
		'fieldname' => 'WorkCode'
		, 'fieldlabel' => 'Project'
		, 'query' => 'SELECT ID, Concat(IFNULL(Projectnummer,\'\'), \' \', Description) AS ProjectNumberName FROM Workcodes WHERE ( isdisabled = 0 AND (lastdate IS NULL OR lastdate = \'\' OR lastdate >= \'' . $oDate->get("Y-m-d") . '\') ) ' . $currentValueOnNew . ' ORDER BY Projectnummer, Description '
		, 'id_field' => 'ID'
